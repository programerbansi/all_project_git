...
import { GET_TYPE_LIST, getDataList, getTypeList } from '../../redux/actions/EcommerceAction';
...
const AllTypes = () => {
    ...
    const typelastpage = useSelector((state) => state.EcommerceReducer.typelastpage);

    const classes = useStyles();
    const location = useLocation();
    const [open, setOpen] = useState(false);
    const [currentPage, setCurrentPage] = useState(1);
    const [deleteItem, setDeleteItem] = useState({ id: '', lastPage: typelastpage, openDeleteDialog: false });
    const [editItem, setEditItem] = useState('');
    const [action, setAction] = useState('');

    const dispatch = useDispatch();
    const fetchData = async () => {
        dispatch(getDataList(`gettype?page=${currentPage == 0 ? 1 : currentPage}`,GET_TYPE_LIST, val.setOpenLoader));
    };
    const query = useQuery(['tag', currentPage], fetchData);
    const fetchData1 = async () => {
        if (currentPage > typelastpage && currentPage != 1)  dispatch(getDataList(`gettype?page=${typelastpage}`,GET_TYPE_LIST, val.setOpenLoader))
    };
    const query1 = useQuery(['tagLastPage', (currentPage > typelastpage && currentPage != 1)], fetchData1);
    // let typeListLoaded = false;

    // useEffect(() => {
    //     if (!typeListLoaded) {
    //         if (currentPage > typelastpage && currentPage != 1) {
    //             dispatch(getDataList(`gettype?page=${typelastpage}`,GET_TYPE_LIST, val.setOpenLoader))
    //             setCurrentPage(typelastpage)
    //         }
    //         else {
    //             dispatch(getDataList(`gettype?page=${currentPage == 0 ? 1 : currentPage}`,GET_TYPE_LIST, val.setOpenLoader));
    //         }
    ...
    // }, [currentPage,(currentPage > typelastpage && currentPage != 1)])
    ...
    return (
        <>
            ...
            <DialogBox obj={{ open: open, setOpen: setOpen, path: location.pathname, name: 'Type', editItem: editItem, action: action, currentPage: currentPage == 0 ? 1 : currentPage }} />
                        ...
                        <TableContainer component={Paper}>
                            <Table sx={{ minWidth: 650 }} aria-label="simple table">
                                ...
                                <TableBody>
                                            ...
                                            <TableCell align="center">
                                                <Tooltip title='Delete'>
                                                    <IconButton edge="end" aria-label="delete" style={{ color: '#ff6363' }} onClick={() => {
                                                        setDeleteItem({ id: item.id, lastPage: typelastpage, openDeleteDialog: true, name: 'type' });
                                                    }
                                                    }>
                                                        <DeleteIcon />
                                                    </IconButton>
                                    ...
                                    {
                                        typelastpage && <TableRow>
                                            <TableCell colSpan={'16'}>
                                                {typelastpage > 1 ? <List>
                                                    <ListItem sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                                                        <Stack>
                                                            <Pagination siblingCount={0} boundaryCount={0} count={typelastpage} color="primary" onChange={(event, value) => setCurrentPage(value)} variant="outlined" page={currentPage || 1} />
                                                        </Stack>
                                                    </ListItem>
                                                </List> : null}
                                            </TableCell>
